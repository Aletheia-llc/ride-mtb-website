// Import bike shops from Foursquare Places API (new Service API)
// Category 4bf58dd8d48988d115951735 = Bicycle Store
// Grids the US into ~500 overlapping cells (100km radius each)
// Safe to re-run — uses ON CONFLICT DO NOTHING on osmId
//
// Usage: FOURSQUARE_API_KEY=xxx node scripts/sync-foursquare-shops.mjs
...
const BIKE_SHOP_CATEGORY = '4bf58dd8d48988d115951735'
...
async function searchCell(lat, lon) {
  ...
  url.searchParams.set('categories', BIKE_SHOP_CATEGORY)
...
async function main() {
  ...
  console.log('Starting Foursquare import (new Places API)...')
  ...
  for (let i = 0; i < grid.length; i++) {
    ...
    for (const place of results) {
      ...
      // Only insert actual bike shops — skip fallback/unrelated results
      const isBikeShop = place.categories?.some(c => c.fsq_category_id === BIKE_SHOP_CATEGORY)
      if (!isBikeShop) continue
      ...
      // New API: lat/lon are top-level, not nested under location.geocodes
      const placeLat = place.latitude ?? null
      const placeLon = place.longitude ?? null
      if (!placeLat || !placeLon) { skipped++; continue }

      const loc = place.location ?? {}
      // New API: state is under location.region (2-letter abbreviation)
