...
const step4Schema = z.object({
  yearsRiding: z.coerce.number().int().min(0).max(50).optional().nullable(),
...
export async function saveStep(
  ...
  try {
    switch (step) {
      ...
      case 4: {
        const yearsRaw = formData.get('yearsRiding')
        const data = step4Schema.parse({
          yearsRiding: yearsRaw ? (yearsRaw as string) : undefined,
        ...
        if (data.yearsRiding !== undefined && data.yearsRiding !== null) {
          updateData.yearsRiding = data.yearsRiding
        }
