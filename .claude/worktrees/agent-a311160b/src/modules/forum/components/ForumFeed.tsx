...
import type { ForumThreadSummary } from '@/modules/forum/types'
...
export function ForumFeed({ threads, categorySlug }: ForumFeedProps) {
  if (threads.length === 0) {
    return (
      <EmptyState
  ...
  return (
    <div className="divide-y divide-[var(--color-border)]">
      {threads.map((thread) => {
        ...
        const replyCount = thread._count.posts - 1 // subtract the OP post

        return (
          <article
            key={thread.id}
            className={`flex items-center gap-4 px-2 py-3 transition-colors hover:bg-[var(--color-bg-secondary)] ${
              thread.isPinned ? 'bg-[var(--color-bg-secondary)]/50' : ''
            }`}
          >
            {/* Author avatar */}
            {author && (
