    ...
    <div className="mx-auto flex max-w-2xl flex-col gap-6">
      ...
      >
        ...
        <ul className="flex flex-col gap-1.5">
          {result.whyMatches.map((reason, i) => (
            <li key={i} className="relative pl-4 text-sm leading-relaxed text-[var(--color-text-muted)]">
              <span className="absolute left-0 text-xs font-semibold text-[var(--color-success)]">✓</span>
              {reason}
            </li>
          ))}
        </ul>
              ...
              return (
                ...
                >
                  <div className="relative aspect-square overflow-hidden">
                    ...
                    />
                    <h3 className="absolute inset-x-0 bottom-0 z-10 p-3 text-base font-bold text-white">
                      {alt.categoryName}
                    </h3>
                  </div>
      ...
      <div className="flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-secondary)] px-4 py-2 text-sm font-medium text-[var(--color-text)] transition-colors hover:border-[var(--color-primary)]"
        >
          {copied ? <Check className="h-4 w-4 text-[var(--color-success)]" /> : <Share2 className="h-4 w-4" />}
