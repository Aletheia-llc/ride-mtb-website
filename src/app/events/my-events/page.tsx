...
export default async function MyEventsPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
      ...
      {rsvps.length === 0 ? (
        <p className="text-sm text-[var(--color-text-muted)]">You haven&apos;t RSVPed to any events yet. <a href="/events/search" className="text-[var(--color-primary)] hover:underline">Find events</a>.</p>
      ) : (
        <div className="space-y-3">
          {rsvps.map((rsvp) => (
            <a key={rsvp.id} href={`/events/${rsvp.event.slug}`}
